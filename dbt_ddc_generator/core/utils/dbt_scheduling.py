                            ...
                            if pipeline_config and isinstance(pipeline_config, dict):
                                ...
                                for model in pipeline_config.get('models', []):
                                    if model.get('name') == model_name:
                                        return {
                                            'file_path': file_path,
                                            'pipeline_name': os.path.basename(root),  # Using directory name as pipeline name
                                            'schedule': pipeline_config.get('schedule_interval'),
                                            'owner': pipeline_config.get('owner'),
                                            'description': pipeline_config.get('description'),
                                            'model_config': model,
                                            'profile': pipeline_config.get('profile'),
                                            'notify_on_failure': pipeline_config.get('notify_on_failure'),
                                            'tags': pipeline_config.get('tags'),
                                            'deploy_env': pipeline_config.get('deploy_env')
                                        }
